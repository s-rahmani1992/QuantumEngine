    ...
    for (UINT i = 0; i < desc.BoundResources; ++i) {
        ...
        if (boundResource.Type == D3D_SIT_CBUFFER)
        {
            ...
            if (cbDesc.Size / cbDesc.Variables <= sizeof(Matrix4)) { //TODO Find better condition for constant buffer checking if possible
                Shader::RootConstantBufferData cData{
                ...
                for (UINT v = 0; v < cbDesc.Variables; ++v) {
                    ...
                    cData.rootConstants.push_back(Shader::RootConstantVariableData{
                ...
                m_reflection.resourceVariables.push_back(Shader::ResourceVariableData{
                    .rootParameterIndex = rootParamIndex,
                    .name = name,
                    .resourceData = boundResource });
            }
        ...
        // Sampler is considered as separate type because it can either contribute as root parameter or as static sampler
            ...
            m_reflection.samplerVariables.push_back(Shader::ResourceVariableData{ .rootParameterIndex = 9999, .name = name, .resourceData = boundResource });
        }
        else {
            m_reflection.resourceVariables.push_back(Shader::ResourceVariableData{ .rootParameterIndex = rootParamIndex, .name = name, .resourceData = boundResource });
            rootParamIndex++;
        }
    }
}
